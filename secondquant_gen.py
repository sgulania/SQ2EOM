...
def contraction(a, b):
    ...
    A contraction is non-zero only if a creator is to the right of a
    annihilator:

    >>> contraction(AnnihilateFermion(p),CreateFermion(q))
    KroneckerDelta(p, q)
    >>> contraction(CreateFermion(p),AnhilateFermion(q))
    0
